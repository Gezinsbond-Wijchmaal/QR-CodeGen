...
def create_custom_theme():
    ...
    start.theme_add_new('MyCustomTheme', DO)
...
layout = [
    [start.Text('Welkom bij de DO Tool', font=fbig, text_color=kleur)],
    [start.Text("Deze tool is gemaakt voor Digitaal Ondertekenen.", font=fontie, text_color=kleur)],
    [start.Text("Ben jij een medewerker van Digitaal Ondertekenen", font=fontie, text_color=kleur)],
...
window = start.Window('DOTool', layout, grab_anywhere=True)
...
while True:
    try:
        ...
        if values['NEE']:
            try:
                start.popup("Goe bezig!", font=fontie, text_color=kleur)
                window.close()
            except Exception as e:
                start.popup(f"Een onverwachte fout is opgetreden bij het sluiten van het venster: {e}", font=fontie, text_color=kleur)
