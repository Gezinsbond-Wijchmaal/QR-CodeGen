...
def create_custom_theme():
    ...
    gbw.theme_add_new('MyCustomTheme', DO)

# RGB naar hexadecimaal
def rgb_to_hex(rgb):
    return '#%02x%02x%02x' % rgb
green_hex = rgb_to_hex((0, 145, 162))
...
font_path = 'fonts/FlandersArtSans-Bold.ttf'
font_size = 16
...
layout = [
    ...
    [gbw.Text('Welke tekst wil je onder QR-Code?', font=(font_path, font_size))],
...
while True:
    ...
    frame = PIL.Image.new(mode="RGB", size=(700, 700),
                    color=(0,145,162))
    ...
    fontie= ImageFont.truetype('fonts/FlandersArtSans-Bold.ttf', 60)
    fonts= fontie
    ...
    if ratiox > max_ratiox:
        ...
        fonts = ImageFont.truetype('fonts/FlandersArtSans-Bold.ttf', new_font_sizex)
    

    draw.text((32,609), "",font=fontie, fill=(255,255,255))
    ...
    fontie= ImageFont.truetype('fonts/FlandersArtSans-Bold.ttf', 120)
    fontje= fontie
    ...
    if ratioj > max_ratioj:
        ...
        # Maak een nieuw font object met de nieuwe font size
        fontje = ImageFont.truetype('fonts/FlandersArtSans-Bold.ttf', new_font_sizej)
    ...
    drawtje.text ( (text_x, text_y), afdeling, font=fontje, fill=(0,145,162))
    gb = gbz
    ...
    qrtje.add_data(url)
    qrtje.make()
    qrtje = qrtje.make_image(fill_color="#0091A2", back_color="#ffffff").convert('RGB')
