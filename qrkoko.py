...
layout = [  [sg.Text('Selecteer de map waar het moet opgeslagen worden')],
            [sg.In(key='Mapje'), sg.FolderBrowse()],
            [sg.Text('Vul hier de URL in')],
            [sg.InputText(key='URL')],
            [sg.Text('Vul hier de naam van het bestand in (in 1 woord)')],
            [sg.InputText(key='Bestand')],
            [sg.Text('Wat is de naam van de afdeling?')],
            [sg.InputText(key='Afdeling')],
            [sg.Text('Welke tekst wil je onder Gezinsbond hebben staan onder de QR-Code?')],
            [sg.InputText(key='Subtekst')],
            [sg.Checkbox(key='geenpopup', text='Geen popup na aanmaken van het bestand', default=False)],
            [sg.Button('Ok'), sg.Button('Sluiten')] ]
...
while True:
    ...
    if event == sg.WIN_CLOSED or event == 'Sluiten': # if user closes window or clicks cancel
        break
    print(values['URL'])
...
>>>>>>> 4dd12b340ed3b055fd07f7774535d4beef2d3531
window.close()